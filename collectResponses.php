...
<?php
	...
	{
		if ( SectionnameToURLName($section['SectionName']) == eliminar_tildes($currentSection))
		{
			$sectionIndex = $index;
		}
	}
	...
	<div class="container">
		...
		<div class="row form-group">
			<div class="text-center col-12">
				...
				<?php if ($nextSection == '') { ?>
					<button type="submit" class="btn btn-primary" onclick="$('form').attr('action', 'devops-maturity-csv.php');">View Results</button>
				<?php } ?>
			</div>
		</div>
	</div>
...
<!-- Question-Card -->
	...
	function renderText($question) { 
		...
		foreach ($question['Answers'] as $index=>$answer) { ?>
			<div class="custom-control my-2">
			<input type="text" name="<?=$answer['ID']?>" value="<?=$answer['Value']?>" />
